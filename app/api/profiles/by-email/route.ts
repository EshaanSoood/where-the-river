...
export async function POST(req: Request) {
  try {
    ...
    const name = [um.first_name, um.last_name].filter(Boolean).join(" ").trim() || (um.name || "").trim();
    ...
    return NextResponse.json({ exists: true, profile: { id: (authUser as unknown as AuthUserRow).id, name, ref_code_8: ref_code_8 } });
  } catch (e: unknown) {
